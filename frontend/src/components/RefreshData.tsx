...
export default function RefreshData({
  ...
  return (
    <div ref={rootRef} className="group relative flex items-center gap-2.5">
      ...
      <button
        type="button"
        onClick={() => (sweeping ? setOpen((v) => !v) : runIngest(false))}
      ...
      {/* Source-cadence tooltip — small, hover only. Yields to the
          popover so the two never stack. */}
      {showPopover ? null : (
        <div className="absolute right-0 top-full z-20 mt-2 hidden w-[300px] rounded-[12px] border border-hairline bg-white p-3 shadow-panel group-hover:block">
