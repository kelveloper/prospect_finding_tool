...
type Props = {
  title: string;
  /** Short line under the title saying what is inside, so the advisor can
   *  decide whether to open it without opening it. */
  hint?: string;
  /** Count shown beside the title, e.g. how many rows are inside. */
  badge?: string;
  defaultOpen?: boolean;
  /** Overrides the body padding — nested cards want a tinted, tighter body. */
  contentClassName?: string;
  children: ReactNode;
};

/** A native <details> panel. No JavaScript, works before hydration, and the
 *  browser gives us keyboard and screen-reader behaviour for free. */
export default function Collapsible({
  ...
  return (
    ...
    >
      ...
      <div className={"border-t border-surface-soft " + contentClassName}>
        {children}
      </div>
    </details>
  );
