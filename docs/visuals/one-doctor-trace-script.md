...
- "Each answers a question no other source can. **Licensing** turns a claim into a fact and starts the clock. **Medicare** doesn't tell us what she makes — it tells us whether she's the employee or the employer; for orthopaedics specifically that's close to universal, ~99% participate. The real limit isn't opt-out, which is low everywhere — it's that Medicare statutorily excludes cosmetic procedures from coverage, so a heavily cosmetic practice can look quiet on this signal while fully enrolled. We've already priced the fix — a business-registry lookup by person name that finds ownership without touching Medicare billing at all — it's built and tested, just not switched on; it's paid. **Deeds** are the only window into her personal life — a fresh mortgage is the moment people actually call an advisor."
...
**"Does Medicare/PECOS work the same for every specialty?"**
"Not quite, and now we've checked. Formal opt-out is low everywhere but does vary — dermatology sits around 1.8%, versus under 1% overall and about 7% for psychiatry, the highest opt-out specialty. But opt-out isn't the mechanism that actually threatens this signal: Medicare statutorily excludes cosmetic procedures from coverage, for every physician, enrolled or not. A dermatologist can be fully Medicare-enrolled and still generate almost no billing or reassignment record if her practice leans cosmetic — Botox, fillers, laser — because those services were never a covered claim to begin with, not because she opted out. Dr Reyes is orthopaedics, where the signal is close to universal; the real risk concentrates in cosmetic-leaning specialties, and that's a genuine limit of the tool worth saying out loud, not papering over."

**"So what are you doing about the cosmetic-specialty gap?"**
"We already built and tested the fix, it's just not switched on. A Secretary-of-State business-registry lookup, searched by the physician's own name, finds practice ownership as a legal filing — a formation event — completely independent of whether Medicare ever sees a claim. We validated it against 7 real physician-owned practices PECOS already found, and it matched clean. It also solves a second problem: PECOS can tell us *that* someone owns a practice, never *when* — a business filing carries the formation date directly, which is the timing signal we actually want. It's paid, fifty cents to two dollars a lookup depending on volume, roughly a hundred to four hundred dollars for a cohort. We had a free mock version of this adapter in August and pulled it in favor of the real PECOS-based signal once we confirmed there's no free machine-readable feed — the paid path is scoped, costed, and sequenced in `RESEARCH_COMMERCIAL_SOURCES.md`, just not funded yet."
...
KFF (Nov 2024): 98% of non-pediatric physicians participate; 1.2% opted out overall. Opt-out concentrates by specialty — psychiatry 8.1%, plastic surgery 4.5%, neurology 3.2%. CDC MMWR: ~99% of orthopaedic surgeons accept Medicare. JMIR Dermatology (data through May 2022): dermatology opt-out ~1.8% (196/11,003), vs ~7.0% psychiatry and ~0.5% otolaryngology in that same study — the two studies use different years/methods, so quote the specialty comparison within one study, not across them. CMS coverage policy (LCD L39506, cosmetic and reconstructive surgery): cosmetic procedures are statutorily excluded from Medicare coverage for every participating physician — this, not opt-out, is the mechanism that actually limits the signal for cosmetic-leaning practices.

**Business-registry mitigation, if pressed for detail**
Vendor: Cobalt Intelligence, live Secretary-of-State pulls across all 50 states + DC, searchable by person name, returns formation date + officers + status (22 fields for Illinois). $0.50–2.00/lookup, 20 free trial lookups, pay-as-you-go, no contract. Already validated: 7 PECOS-found physician-owned entities looked up clean. A free version (`app/adapters/il_sos`, mock data) existed 2026-08-18 to 2026-08-23, then was deleted once research confirmed IL SoS has no free bulk/API access (phone-contract sales only) and OpenCorporates' free tier ended. Full writeup: `docs/RESEARCH_COMMERCIAL_SOURCES.md`, Pillar 3.
